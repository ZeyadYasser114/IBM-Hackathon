 ...
 */
function snippetId(changeSetId: string, filePath: string, startLine: number, content: string): string {
...
function deduplicateSnippets(snippets: EvidenceSnippet[]): EvidenceSnippet[] {
  // Group by file path then sweep for overlaps.
  const byFile = new Map<string, EvidenceSnippet[]>();
...
function diffContextSnippets(
  ...
  if (added) snippets.push(added);

  const removed = build(removedLines, "removed");
  if (removed) snippets.push(removed);
...
function structuralSnippets(
  ...
  if (touchedNewLines.length === 0) return [];
  ...
  for (const touchedLine of touchedNewLines) {
    ...
    for (let i = touchedIdx; i >= scanStart; i--) {
      const cl = classifyLine(fileLines[i] ?? "");
      if (cl) {
        openerIdx = i;
        classification = cl;
        break;
    ...
    if (openerIdx === null || classification === null) continue;

    // Find where this block ends.
    let endIdx: number;
    if (classification.opensBlock) {
      endIdx = findBlockEnd(fileLines, openerIdx, maxLines) ?? Math.min(openerIdx + maxLines - 1, fileLines.length - 1);
    } else {
      endIdx = openerIdx;
    }
    ...
    let diffRelation: EvidenceDiffRelation;
    if (hasAdded && hasRemoved) diffRelation = "added"; // mixed — default to "added"
    else if (hasAdded) diffRelation = "added";
    else if (hasRemoved) diffRelation = "removed";
    else diffRelation = "surrounding";
...
function extractFromFile(
  ...
  const canExtractStructural = fileLines !== undefined && isTsJsFile(file.path);

  for (const hunk of file.hunks) {
    try {
      let hunkSnippets: EvidenceSnippet[] = [];

      if (canExtractStructural && fileLines) {
        hunkSnippets = structuralSnippets(
          changeSetId, file, hunk, fileLines, options.maxSnippetLines,
        );
      }

      // Always add diff-context snippets when structural extraction yields nothing.
      if (hunkSnippets.length === 0) {
        hunkSnippets = diffContextSnippets(
          changeSetId, file, hunk, options.diffContextLines,
        );
      }
      ...
      snippets.push(
        ...diffContextSnippets(changeSetId, file, hunk, options.diffContextLines),
      );
    }
  }
...
export function extractEvidence(
   ...
   */
  headContents: ReadonlyMap<string, string> = new Map(),
  ...
  const allSnippets: EvidenceSnippet[] = [];

  for (const file of changeSet.changedFiles) {
    const headContent = headContents.get(file.path);
    const fileSnippets = extractFromFile(
      changeSet.id, file, headContent, opts, warnings,
    );
