...
function name(match: RegExpMatchArray, group = 1): string {
 ...
 */
export function classifyLine(line: string): LineClassification | null {
  ...
  let m = trimmed.match(
    /^export\s+default\s+(?:async\s+)?function\s+(\w+)\s*\(\s*req/,
  );
  if (m) {
    return {
      sourceType: "route",
  ...
  m = trimmed.match(
    /^(?:\w+\.(?:get|post|put|patch|delete|head|options|use|all)\s*\()['"`](\/[^'"` ]*)/,
  );
  if (m) {
    return {
      sourceType: "route",
