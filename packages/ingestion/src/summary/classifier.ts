...
const EXT_MAP: ReadonlyArray<[RegExp, FileLanguage]> = [
 ...
 */
export function isAuthOrSecurityPath(filePath: string): boolean {
...
export function computePriority(input: PriorityInput): {
  ...
  const hasRoutes = input.files.some((f) =>
    f.touchedSymbols.some((s) => s.startsWith("route ") || s.startsWith("handler ")),
  );
  const hasAuthPath = input.files.some((f) => isAuthOrSecurityPath(f.filePath));
  const hasSchemaFile = input.files.some(
    (f) =>
      ["graphql", "prisma", "sql", "json", "yaml"].includes(f.language) &&
      !f.isTestFile &&
      !f.isConfigFile,
  );
  ...
  if (hasTypeDecls)     reasons.push("type-declarations-changed");
  if (hasRoutes)        reasons.push("api-routes-changed");
  if (hasAuthPath)      reasons.push("auth-or-security-path");
  if (hasSchemaFile)    reasons.push("schema-file-changed");
  if (hasExportChanges) reasons.push("exports-changed");
  if (hasImportChanges) reasons.push("imports-changed");
  if (highVolume)       reasons.push("high-diff-volume");
  if (allTest)          reasons.push("test-only-changes");
  if (allConfig)        reasons.push("config-only-changes");
  if (allDocs)          reasons.push("documentation-only");
  ...
  if (allTest && !allConfig) return { priority: "low", reasons };
  if (allConfig)             return { priority: "low", reasons };
  if (allDocs)               return { priority: "low", reasons };
